...
void kanonizator::kanon(const kvadrat& lk, std::list<izomorfizm>& avtom, kvadrat* kanf){
	...
	kf[(por << 1) + 1] = por;
	for(int i = 0; i < kol; i++) obrabotka(trans[ind + i], kf, avtom);
	izomorfizm vozv = ~avtom.front();
	for(auto q = avtom.begin(); q != avtom.end(); q++) *q = *q * vozv;
	if(kanf) *kanf = kf;
}
...
void kanonizator::get_invar(const kvadrat* sec_lk[], invariant& invar, sootv& nom){
	...
	std::array<std::pair<invariant_2, int>, 3> temp_3;
	for(int k = 0; k < 3; k++){
		for(int i = 0; i < por; i++){
			...
			temp_2[i] = std::make_pair(temp_1, i);
		}
		...
		temp_3[k] = std::make_pair(invar[k], k);
	}
	std::sort(temp_3.begin(), temp_3.end());
	for(int k = 0; k < 3; k++){
...
void kanonizator::postr_formu(const kvadrat& lk, const morfizm& per, int perv, int& count, int sek){
	...
	iformy[count].parastrof = sek;
	iformy[count].izotop[1] = per;
	iformy[count].izotop[2] = tempm;
	for(int i = 0; i < por; i++) tempm[tempk[i * por]] = i;
	iformy[count].izotop[0] = tempm;
...
void kanonizator::postr_sdvigi(const kvadrat& lk, int perv, int vtor, int lin, int& count, int sek){
	...
	for(int i = 0; i < kol; i++){
		...
		postr_formu(lk, tempm, perv, count, sek);
	}
}
	...
	for(int k = 0, c = 0, sek_p; k < krat_s; k++){
		...
		for(int i = 0, perv_p; i < krat_r; i++){
			...
			for(int j = 0, vtor_p; j < krat_b; j++){
				...
				postr_sdvigi(*sec_lk[sek_p], perv_p, vtor_p, lin, c, sek_p);
				postr_sdvigi(inv_lk[sek_p], perv_p, vtor_p, lin, c, sek_p + 3);
			}
		}
	}
}
...
void kanonizator::obrabotka(const int* perest, kvadrat& kf, std::list<izomorfizm>& avtom){
	...
	for(int i = 2, ii; i < por - 1; i++){
		...
		for(int j = 1, ij; j < por - 1; j++){
			...
			if(x > minz){
				avtom.clear();
				get_avtom(spisok[index][--count[index]], perest, ob_perest, tempizo);
				avtom.push_back(tempizo);
				for(int i = 2; i < por; i++) for(int j = 1; j < por; j++){
					kf[i * por + j] = ob_perest[formy[spisok[index][count[index]]][perest[i] * por + perest[j]]];
				}
				if(!count[index]) return;
	...
	for(int i = 0; i < count[index]; i++){
		get_avtom(spisok[index][i], perest, ob_perest, tempizo);
		avtom.push_back(tempizo);
	}
}
...
void kanonizator::get_avtom(int nom_formy, const int* perest, const int* ob_perest, izomorfizm& tempizo){
	...
	for(int i = 0; i < por; i++){
		...
		tempizo.izotop[1][i] = iformy[nom_formy].izotop[1][perest[i]];
		tempizo.izotop[2][i] = ob_perest[iformy[nom_formy].izotop[2][i]];
	}
}
