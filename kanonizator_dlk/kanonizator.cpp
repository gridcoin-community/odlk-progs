...
unsigned long Kanonizator_dlk::hash_f(const Kanonizator_dlk::morfizm& a){
	...
	if(!((temp2 = hash_tabl[temp &= 0xfffff]) & 0x40000000))
		return hash_tabl[++temp + bool(besp & temp2 & 0xfff)];
	mod = temp2 & 0x1f;
	temp2 = (temp2 >> 5) & 0x1f;
	return hash_tabl[++temp + ((besp ^ (besp >> temp2)) & mod)];
}

int Kanonizator_dlk::kanon(kvadrat& dlk, kvadrat& kf){
	normaliz(dlk);
	morfizm perest;
...
void Kanonizator_dlk::perebor(kvadrat& kf, int klass){
	kvadrat formy[2];
...
void Kanonizator_dlk::obrabotka(const int* perest, const int* ob_perest, const kvadrat formy[], int chform, kvadrat& kf){
